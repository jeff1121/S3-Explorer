...
class ObjectService {
  ...
  Future<List<ObjectNode>> listObjects(String bucket, {String? prefix}) =>
      client.listObjects(bucket, prefix: prefix);
  ...
  Future<TransferTask> upload(
    ...
    final uploader = MultipartUploader(
        client: client, maxConcurrent: client.profile.concurrency);
    return queue.enqueue(
      type: TransferType.upload,
      source: EndpointRef(localPath: localPath, profileId: client.profile.id),
      target:
          EndpointRef(bucket: bucket, key: key, profileId: client.profile.id),
      ...
      executor: (task, control) async {
        ...
        if (useMultipart) {
          ...
          control
              .reportProgress(initial.copyWith(bytesTransferred: totalBytes));
        } else {
          final transferred =
              await client.uploadObject(bucket, key, localPath: localPath);
          control
              .reportProgress(initial.copyWith(bytesTransferred: transferred));
        }
      },
    );
  }
  ...
  Future<TransferTask> download(
    ...
    return queue.enqueue(
      ...
      executor: (task, control) async {
        ...
        control.reportProgress(initial.copyWith(bytesTransferred: 0));
        final transferred =
            await client.downloadObject(bucket, key, localPath: localPath);
        control.reportProgress(initial.copyWith(
            bytesTransferred: transferred, totalBytes: transferred));
      },
    );
  }

  Future<TransferTask> copy(String sourceBucket, String sourceKey,
      String destinationBucket, String destinationKey) async {
    final initial = const TransferProgress(totalBytes: 1);
    return queue.enqueue(
      type: TransferType.copy,
      source: EndpointRef(
          bucket: sourceBucket, key: sourceKey, profileId: client.profile.id),
      target: EndpointRef(
  ...
  Future<TransferTask> move(String sourceBucket, String sourceKey,
      String destinationBucket, String destinationKey) async {
    final initial = const TransferProgress(totalBytes: 1);
    return queue.enqueue(
  ...
  Future<void> deleteObjects(String bucket, List<String> keys) =>
      client.deleteObjects(bucket, keys);

  Future<String> downloadToDirectory(
      String bucket, ObjectNode node, String directoryPath) async {
