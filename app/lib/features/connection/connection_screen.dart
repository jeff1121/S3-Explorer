                    ...
                    children: [
                      ...
                      Expanded(
                        child: ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ListTile(
                              tileColor:
                                  selected ? NowUITheme.surface : Colors.white,
                              shape: RoundedRectangleBorder(
                                  borderRadius: BorderRadius.circular(8)),
                              title: Text(profile.name),
                              subtitle: Text(
                                  '${profile.endpoint.host} (${profile.region})'),
                              trailing: selected
                                  ? const Icon(Icons.check_circle,
                                      color: NowUITheme.primary)
                                  : null,
                      ...
                      Row(
                        children: [
                          ...
                          TextButton.icon(
                            onPressed:
                                vm.selected == null ? null : vm.deleteSelected,
                            ...
                            children: [
                              ...
                              Expanded(
                                child: TextFormField(
                                  ...
                                  decoration: const InputDecoration(
                                      labelText: '預設 Bucket (選填)'),
                                  onChanged: (v) =>
                                      vm.updateDraft(defaultBucket: v),
                                ),
                              ),
                            ],
                          ),
                          ...
                          Row(
                            children: [
                              Expanded(
                                child: TextFormField(
                                  key: ValueKey(
                                      'concurrency-${vm.selected?.id}'),
                                  initialValue: vm.concurrency.toString(),
                                  decoration:
                                      const InputDecoration(labelText: '並行數'),
                                  keyboardType: TextInputType.number,
                                  onChanged: (v) => vm.updateDraft(
                                      concurrency:
                                          int.tryParse(v) ?? vm.concurrency),
                                ),
                              ),
                              const SizedBox(width: 12),
                              Expanded(
                            ...
                            children: [
                              Expanded(
                                child: TextFormField(
                                  ...
                                  decoration:
                                      const InputDecoration(labelText: '重試次數'),
                                  keyboardType: TextInputType.number,
                                  onChanged: (v) => vm.updateDraft(
                                      retryMaxAttempts: int.tryParse(v) ??
                                          vm.retryMaxAttempts),
                                ),
                              ),
                              const SizedBox(width: 12),
                              Expanded(
                                child: TextFormField(
                                  ...
                                  onChanged: (v) => vm.updateDraft(
                                      retryBackoffMs:
                                          int.tryParse(v) ?? vm.retryBackoffMs),
                                ),
                              ),
                            ],
                          ),
